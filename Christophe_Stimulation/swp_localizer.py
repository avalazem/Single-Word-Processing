...
# Initialize the runs dictionary for different stimulus types
runs = {
    "audio": [],
    "images": [],
    "covert_speech": [],
...
for i, (run_name, run_data) in enumerate(runs.items()):
    ...
    for stimulus in run_data:
        cue = stimuli.FixCross(size=(50, 50), line_width=4)
        cue.present()
        exp.clock.wait(500)  # Fixation duration (500 ms)

        # Determine the type of stimulus and present it accordingly
        if run_name == "images":
             
            # Show the image
            image_path = os.path.join(instruction_image_folder, stimulus)
            ...
            exp.clock.wait(VISUAL_WAIT_DURATION)
